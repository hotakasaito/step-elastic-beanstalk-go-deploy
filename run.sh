...
export AWS_ACCESS_KEY_ID=$WERCKER_ELASTIC_BEANSTALK_GO_DEPLOY_KEY
export AWS_SECRET_ACCESS_KEY=$WERCKER_ELASTIC_BEANSTALK_GO_DEPLOY_SECRET
...
AWSAccessKeyId=$WERCKER_ELASTIC_BEANSTALK_GO_DEPLOY_KEY
AWSSecretKey=$WERCKER_ELASTIC_BEANSTALK_GO_DEPLOY_SECRET
EOT
...
branch-defaults:
  default:
    environment: $WERCKER_ELASTIC_BEANSTALK_GO_DEPLOY_ENV_NAME
  $WERCKER_GIT_BRANCH:
    environment: $WERCKER_ELASTIC_BEANSTALK_GO_DEPLOY_ENV_NAME
global:
...
/usr/local/bin/eb use $WERCKER_ELASTIC_BEANSTALK_GO_DEPLOY_ENV_NAME || fail "EB is not working or is not set up correctly."
